...
export const validateData = async (data: EventFormData): Promise<Coordinates | string[]> => {
    ...
    if(!data.name || data.name.length < 3 || data.name.length > 50) {
        errors.push(`Nazwa wydarzenia musi mieć od 3 do 50 znaków - obecnie jest ${data.name.length}`);
    }

    if(!data.description || data.description.length < 10 || data.description.length > 500) {
        errors.push(`Opis wydarzenia musi mieć od 10 do 500 znaków - obecnie jest ${data.description.length}`);
    }
